import socket
from detector import Eye_Detector
from Task import Task
import pickle
import multiprocessing
import threading
from imutils import face_utils
from scipy.spatial import distance as dist
import imutils
import dlib
import cv2
import numpy as np
import time
import matplotlib.pyplot as plt
import matplotlib.patches as mpatches
from Task import Task

FRAME_QUEUE = multiprocessing.JoinableQueue()
RESULT_QUEUE = multiprocessing.Queue()

PUPIL_MIN = 15
EYE_AR_THRESH = 0.3  # indicate blink
EYE_AR_CONSEC_FRAMES = 3  # for the number of consecutive frames the eye must be below the threshold
(lStart, lEnd) = face_utils.FACIAL_LANDMARKS_IDXS['left_eye']
(rStart, rEnd) = face_utils.FACIAL_LANDMARKS_IDXS['right_eye']
file = 'Resources/face_landmarks.dat'

OK = b'ok'
HOST = 'localhost'
PORT = 8080

video_p, comm_p = None, None

detector = dlib.get_frontal_face_detector()
predictor = dlib.shape_predictor(file)

detector_params = cv2.SimpleBlobDetector_Params()
detector_params.filterByArea = True
detector_params.maxArea = 1500
blob_detector = cv2.SimpleBlobDetector_create(detector_params)

start_t = 0
left_d = 0.0
right_d = 0.0
counter = 0 # blink count for threshold
blink = 0  # blink total
left_pupil_data = []
right_pupil_data = []
mean = []

pupil_data = [] # (time, left, right, mean)
task = Task()

def extract_eye(frame, shape, start, end):
    (x, y, w, h) = cv2.boundingRect(np.array([shape[start:end]]))
    roi = frame[y:y + h, x:x + w]
    roi = imutils.resize(roi, width=250, inter=cv2.INTER_CUBIC)
    return roi


def get_keypoints(image, threshold):
    image = imutils.resize(image, width=250, inter=cv2.INTER_CUBIC)
    _, image = cv2.threshold(image, threshold, 255, cv2.THRESH_BINARY)
    erode = cv2.erode(image, None, iterations=3)
    dilate = cv2.dilate(erode, None, iterations=5)
    blur = cv2.medianBlur(dilate, 5)
    keypoints = blob_detector.detect(blur)
    return keypoints


def blob_process_both_eyes(left_eye, right_eye, start_thres):
    left_gray = cv2.cvtColor(left_eye, cv2.COLOR_BGR2GRAY)
    right_gray = cv2.cvtColor(right_eye, cv2.COLOR_BGR2GRAY)

    if start_thres == 0:
        end_thres = 130
    else:
        end_thres = start_thres + 10
    for thres in range(start_thres - 10, end_thres):  # start_thres +- 10
        left_keypoints = get_keypoints(left_gray, thres)
        right_keypoints = get_keypoints(right_gray, thres)
        if len(left_keypoints) > 0 and len(right_keypoints) > 0:
            return thres, left_keypoints, right_keypoints
    return None, None, None


def eye_aspect_ratio(eye):
    A = dist.euclidean(eye[1], eye[5])  # vertical
    B = dist.euclidean(eye[2], eye[4])

    C = dist.euclidean(eye[0], eye[3])  # horizontal
    ear = (A + B) / (2.0 * C)
    return ear


def show_text(frame, ear, start_t):
    cv2.putText(frame, "Blinks: {}".format(blink), (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1)
    cv2.putText(frame, "EAR:{:.2f}".format(ear), (10, 50),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1)
    cv2.putText(frame, 'Left Pupil: {:.3f}'.format(left_d), (300, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1)
    cv2.putText(frame, 'Right Pupil: {:.3f}'.format(right_d), (300, 50),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1)
    cv2.putText(frame, 'Time {:2f}'.format(time.time() - start_t), (10, 350),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 1)

def run_video():
    cap = cv2.VideoCapture(0)
    return cap

def end_video(cap):
    cap.release()
    cv2.destroyAllWindows()

def start_detector(cap):
    if not cap:
        print('video is not up')
        return
    current = -1
    threshold = 0
    while True:
        _, frame = cap.read()
        frame = imutils.resize(frame, width=500, height=500)
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

        rects = detector(gray, 0)
        for rect in rects:
            shape = predictor(gray, rect)
            shape = face_utils.shape_to_np(shape)

            # extract eyes
            left = extract_eye(frame, shape, lStart, lEnd)
            right = extract_eye(frame, shape, rStart, rEnd)
            # detect pupil
            trial_thres, le_keypoints, re_keypoints = blob_process_both_eyes(left, right, threshold)
            if trial_thres != None:
                threshold = trial_thres
            # print('thres', threshold)

            if le_keypoints and re_keypoints:
                ld = le_keypoints[0].size
                rd = re_keypoints[0].size
                if ld < PUPIL_MIN and rd < PUPIL_MIN:
                    left_d = ld
                    right_d = rd
                    m = (ld + rd)/2

                    left_pupil_data.append(ld)
                    right_pupil_data.append(rd)
                    mean.append(m)
                    current += 1

            # detecting blink
            leftEye = shape[lStart: lEnd]
            rightEye = shape[rStart: rEnd]
            leftEAR = eye_aspect_ratio(leftEye)
            rightEAR = eye_aspect_ratio(rightEye)

            ear = (leftEAR + rightEAR) / 2.0

            if ear < EYE_AR_THRESH:
                counter += 1
            else:
                if counter >= EYE_AR_CONSEC_FRAMES:
                    blink += 1
                counter = 0

            show_text(frame, ear, start_t)

        cv2.imshow('Frame', frame)
        key = cv2.waitKey(1) & 0xFF

        if key == ord('q'):
            break


def handle_message(s, msg):

    try:
        print('receive<---', msg)
        msg = msg.decode()

        # start a task
        # set task start_time
        if msg == 'start':
            task.start_time = time.time()
            s.send(OK)
            task.toString()

        # create a new task
        elif msg == 'next':
            print('next task')
            detector.next_task()
            s.send(b'name')
            name = s.recv(4096).decode()
            task.task_name = name
            s.send(OK)
            task.toString()

        elif msg == 'end':
            print('task end')
            send_task(s)

        elif msg == 'close':
            s.send(b'close')
            video_p.terminate()
            exit(0)

    except socket.error as err:
        print(err)


def send_task(s):
    task = detector.get_task()
    task.toString()
    try:
        data = pickle.dumps(task)
        print('done dumps')
        s.sendall(b'[D]'+ data)
        print('send TASK -->')
    except socket.error as err:
        print(err)


if __name__ == '__main__':
    processes = []

    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.setblocking(1)

    try:
        # s.connect((HOST, PORT))
        print('RUNNING CLIENT ---> ')

        cap = cv2.VideoCapture(0)

        video_p = multiprocessing.Process(target=start_detector, args=(cap,))
        video_p.start()
        video_p.join()

        print('DETECTOR UP')

        while True:
            msg = s.recv(4096).decode()
            print(msg)
            if msg:
                print('create new process with new msg -------> ')

                comm_p = threading.Thread(target=handle_message(s, msg))
                print('create new process', comm_p)

                comm_p.start()
                comm_p.join()
                comm_p.terminate()

    except socket.error as err:
        print(err)
